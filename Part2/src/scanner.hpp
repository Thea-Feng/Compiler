...
public:
    void print();
    void optimized();
    bool equal(State* x, State* y, char c);
...
/* Commonly used components */
    ...
    static NFA* from_any_char(int except);
...
public:
    Scanner();
    // std::map< std::pair<DFA::State*, char>, std::set<NFA::State*> > Dtrans;
...
public:   
    ...
    void NFA_to_DFA() {
        dfa->states = nfa->to_DFA();
        // print_dfa();
        // printf("Number of States Before OPTIMIZED: %d\n", dfa->states.size());
        dfa->optimized();
        // print_dfa();
        // printf("Number of States Before OPTIMIZED: %d\n", dfa->states.size());
